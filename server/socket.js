...
module.exports = function(io, users, waitingQueue) {
  
  function removeFromQueue(socketId) {
    return waitingQueue.filter(s => s.id !== socketId);
  }
  ...
  io.on("connection", socket => {
    ...
    socket.on("find-partner", () => {
      ...
      if (partner) {
        waitingQueue = removeFromQueue(partner.id);
    ...
    socket.on("respond-request", ({ requestId, accepted, fromId }) => {
      ...
      if (accepted) {
        if (senderSocket && users[fromId] && users[fromId].status !== 'connected') {
          waitingQueue = removeFromQueue(socket.id);
          waitingQueue = removeFromQueue(fromId);
